...
# --- 3. бэкап до правки
#
# В имени каталога время прогона, а не только дата (задание 026). Прежнее имя
# backups/ГГГГММДД-<пакет> повторный прогон переиспользовал и затирал им
# единственный дамп «до»: после проверочного второго прогона откатываться было
# уже некуда, и в заданиях 024–025 дамп приходилось копировать руками.
